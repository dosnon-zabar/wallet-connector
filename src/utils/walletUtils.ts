...
async function fetchValidatorInfo(address: string) {
  try {
    ...
    const validatorDetailsPromises = delegationsData.delegation_responses.map(async (delegation: any) => {
      ...
      // Fetch validator thumbnail
      const infoResponse = await fetch(
        `https://keybase.io/_/api/1.0/user/lookup.json?fields=pictures&key_suffix=${validatorData.validator.description.identity}`,
      )
      const infoData = await infoResponse.json()
      const thumbnail = infoData.them?.[0]?.pictures?.primary?.url || null

      return { ...validatorData, thumbnail }
    })
    ...
    const validatorInfo = delegationsData.delegation_responses.map((delegation: any, index: number) => {
      ...
      const thumbnail = validatorDetails[index]?.thumbnail || null
